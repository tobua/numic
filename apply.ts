...
const collectRejectedHunks = (location: string) => {
  ...
  for (const fileName of rejectedFiles) {
    ...
    if (contents) {
      ...
      const contentsSplitByLine = contents.split(splitByLineRegex)
...
export const applyPatch = (location: string, git: (...args: string[]) => SpawnSyncReturns<Buffer>) => {
  ...
  if (existsSync(rejectedHunksPath)) {
    log('Removing patch/rejected-hunks.patch, assuming appropriate changes have been made in current.patch')
  }
  ...
  if (rejectedHunks.length > 0) {
    ...
    if (rejectedHunks.length > 0) {
      ...
      log('2) Manually apply the proper changes in /ios or /android and run"npx numic patch" to update the patch and then try again')
      return
    }
  }
